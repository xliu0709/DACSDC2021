...
void conv3x3_bn_act_DSPopt_hls_wrapper(
    stream<ap_uint<CONV_1_IN_BIT * CONV_1_INPE>> &in,
  ...
  conv3x3_bn_act_DSPopt<CONV_1_IFM_ROW, CONV_1_IFM_COL, CONV_1_IFM_CH,
                        ...
                        CONV_1_L_SHIFT>(in, weights, inc, bias, out);
}
...
int main(int argc, char **argv) {
  ...
  hls::stream<ap_uint<CONV_1_IN_BIT * CONV_1_SIMD_DSP6>> test_in("test_in");
  ...
  for (int r = 0; r < CONV_1_IFM_ROW; r++) {
    for (int i = 0; i < CONV_1_IFM_CH; i += CONV_1_SIMD_DSP6) {
      for (int c = 0; c < CONV_1_IFM_COL; c++) {
        ap_uint<CONV_1_IN_BIT * CONV_1_SIMD_DSP6> data;
        for (int s = 0; s < CONV_1_SIMD_DSP6; s++) {
  ...
  conv3x3_bn_act_DSPopt_hls_wrapper(test_in, conv_1_w_dspopt, conv_1_inc,
                                    conv_1_bias, test_out);
