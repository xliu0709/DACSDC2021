...
void *load_image_pthread(void * thread_para) {
    ...
    for (int i=para.start; i < para.end; i ++) {
        ...
        Mat resize_image;
        Mat rgb_image;
        resize(image, resize_image, Size(para.col, para.row), 0, 0, INTER_NEAREST);
        cvtColor(resize_image, rgb_image, COLOR_BGR2RGB);
        uint8_t * arr = para.matrix + i * rgb_image.cols * rgb_image.rows * rgb_image.channels();
        mat_to_arr(rgb_image, arr);
    }
...
extern "C" {
    void load_resize_image(char **paths, uint8_t *matrix, int batch_size, int row, int col, int ch) {
